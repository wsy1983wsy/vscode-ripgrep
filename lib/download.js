...
const tmpDir = path.join(
  os.tmpdir(),
  `@opensumi-vscode-ripgrep-cache-${packageVersion}`
);
...
async function getAssetFromCDN(opts, assetName) {
  ...
  // https://gw.alipayobjects.com/os/lib/opensumi/vscode-ripgrep-prebuilt/1.4.0/build/ripgrep-v13.0.0-4-aarch64-apple-darwin/rg.wasm
  const asset = {
    url: `https://gw.alipayobjects.com/os/lib/opensumi/vscode-ripgrep-prebuilt/${packageVersion}/build/${assetName}`,
  };
...
module.exports = async (opts) => {
  ...
  const assetName =
    ["ripgrep", opts.version, opts.target].join("-") + `/${filename}.wasm`;
